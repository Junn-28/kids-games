...
**入力欄（`<input>`）をつくらない。** 端末のキーボードが出ると、こどもだけでは
操作しづらいうえ、漢字も英数字も絵文字も入ってきます。

呼び名が要るときは、ゲームの中にひらがなのボタンを並べて、タップで1文字ずつ
作らせてください（きおくバトルの「なまえ」がこの形です）。このとき、

- 長さの上限を決める
- 使ってよい文字の集合を決めて、`localStorage` から読んだ名前は
  1文字ずつ検証し、外れていたら丸ごと捨てる

の2つは必ずやること。本名やメールアドレスは、どんな形でも聞かないこと。
